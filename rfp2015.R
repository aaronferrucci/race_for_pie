...
do_plot <- function(format_seconds) {

  if (format_seconds)
    x_label <- "elapsed time (h:mm:ss)"
  else
    x_label <- "elapsed time (h:mm)"
  ...
  p10k <- p10k + scale_x_continuous(
    ...
    labels = timestr(elapsed_ticks10k, format_seconds),
    name = x_label
  )
  ...
  p5k <- p5k + scale_x_continuous(
    ...
    labels = timestr(elapsed_ticks5k, format_seconds),
    name = x_label
  )
  ...
  return(grid.arrange(p5k, p10k, ncol=2, top="2015 Run For Pie"))
}
